...
def run_one_case(use_model, **kwargs):
    ...
    for i in range(iterations - 1):
        ...
        mvt.control(ego_veh, cut_in_veh, freq, check, react, i, perception=noise)
    ...
    if mvt.write_perception_trace(
            noise, 'results/perception_trace_cut_in_' + str(use_model) + '.csv', freq):
        print('Perception trace written to results/perception_trace_cut_in_'
              + str(use_model) + '.csv')
