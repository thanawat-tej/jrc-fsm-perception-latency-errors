...
def run_loop():
    ...
    out_dir = 'results/car_following' + mvt.noise_suffix()
    ...
    for model in models_list:
        ...
        lead_speed_deltas = np.arange(0, 31, 5)

        check = model_functions[model][0]
        react = model_functions[model][1]
        initial_dist = model_functions[model][2]

        result_d = {'max_deceleration': [], 'velocity': [], 'lead_speed_delta': [], 'Crash': [], 'initial_distance': [], 'speed_difference': [],
                    'CFS': [], 'PFS': [],
                    'reacted': [], 'ego_speed': [],
        ...
        for max_dec in max_decelerations:
            for ur in velocities:
                for lead_speed_delta in lead_speed_deltas:

                    initial_distance = initial_dist(imaginary_veh, ur / 3.6)

                    init_pos_c = np.array([initial_distance + length, 0])
                    init_pos_ego = np.array([0, 0])
                    init_long_speed_ego = ur / 3.6
                    init_long_speed_c = max((ur - lead_speed_delta) / 3.6, 0)
                    deceleration = gp.g * max_dec

                    cut_in_veh = mvt.create_profile_decel(init_pos_c, deceleration, init_long_speed_c, iterations, freq)
                    ego_veh = mvt.create_profile_decel(init_pos_ego, 0, init_long_speed_ego, iterations, freq)
                    ...
                    # Per-cell perception noise, seeded from scenario params
                    # (NOT the model) for cross-model comparability.
                    noise = mvt.make_perception_noise(seed=(
                        gp.perception_noise_seed, int(round(max_dec * 1000)), int(ur), int(lead_speed_delta)))
                    ...
                    result_d['lead_speed_delta'].append(lead_speed_delta)
                    ...
                    # Perceived vs true state of the lead vehicle at the
