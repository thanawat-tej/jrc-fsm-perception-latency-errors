...
def run_loop(speed_range):
    ...
    out_dir = 'results/cut_in_' + speed_range + '_speed' + mvt.noise_suffix() + '/'
        ...
        for ego_speed in ego_speeds:
            for cut_in_speed in range(10, ego_speed, cut_in_speeds_step):
                ...
                result_d = {'long_dist': [], 'lat_vel': [], 'Crash': [], 'Crash type': [], 'minTTC': [], 'PFS': [],
                            'CFS': [],
                            'reacted': [], 'ego_speed': [],
                            'perceived_gap': [], 'true_gap': [],
                            'perceived_lat_gap': [], 'true_lat_gap': [],
                            'perceived_lat_speed': [], 'true_lat_speed': [],
                            'perceived_speed': [], 'true_speed': [],
                            'entry_ego_speed': [],
                            'entry_perceived_gap': [], 'entry_true_gap': [],
                            'entry_perceived_lat_gap': [], 'entry_true_lat_gap': [],
                            'entry_perceived_lat_speed': [], 'entry_true_lat_speed': [],
                            'entry_perceived_speed': [], 'entry_true_speed': []}
                ...
                for long_dist in long_dists:
                    for lat_vel in lat_vels:
                        ...
                        # Per-cell perception noise: seeded from the scenario
                        # parameters (NOT the model) so every model sees the
                        # same noise realisation on the same scenario.
                        noise = mvt.make_perception_noise(seed=(
                            gp.perception_noise_seed, int(ego_speed),
                            int(cut_in_speed), int(long_dist),
                            int(round(lat_vel * 1000))))
                        ...
                        # Perceived vs true state of the cut-in vehicle at the
                        # closest-approach step (filled for every cell).
                        # 'reacted' tells whether the ego ever braked.
                        result_d['reacted'].append(not ego_veh.safe)
                        result_d['ego_speed'].append(getattr(ego_veh, 'snap_ego_speed', np.nan))
                        result_d['perceived_gap'].append(getattr(ego_veh, 'snap_perc_gap', np.nan))
                        result_d['true_gap'].append(getattr(ego_veh, 'snap_true_gap', np.nan))
                        result_d['perceived_lat_gap'].append(getattr(ego_veh, 'snap_perc_lat', np.nan))
                        result_d['true_lat_gap'].append(getattr(ego_veh, 'snap_true_lat', np.nan))
                        result_d['perceived_lat_speed'].append(getattr(ego_veh, 'snap_perc_lat_speed', np.nan))
                        result_d['true_lat_speed'].append(getattr(ego_veh, 'snap_true_lat_speed', np.nan))
                        result_d['perceived_speed'].append(getattr(ego_veh, 'snap_perc_speed', np.nan))
                        result_d['true_speed'].append(getattr(ego_veh, 'snap_true_speed', np.nan))
                        # lane-entry snapshot (the cut-in's dangerous instant);
                        # NaN if the NPC never entered the ego's lane.
                        result_d['entry_ego_speed'].append(getattr(ego_veh, 'snap_entry_ego_speed', np.nan))
                        result_d['entry_perceived_gap'].append(getattr(ego_veh, 'snap_entry_perc_gap', np.nan))
                        result_d['entry_true_gap'].append(getattr(ego_veh, 'snap_entry_true_gap', np.nan))
                        result_d['entry_perceived_lat_gap'].append(getattr(ego_veh, 'snap_entry_perc_lat', np.nan))
                        result_d['entry_true_lat_gap'].append(getattr(ego_veh, 'snap_entry_true_lat', np.nan))
                        result_d['entry_perceived_lat_speed'].append(getattr(ego_veh, 'snap_entry_perc_lat_speed', np.nan))
                        result_d['entry_true_lat_speed'].append(getattr(ego_veh, 'snap_entry_true_lat_speed', np.nan))
                        result_d['entry_perceived_speed'].append(getattr(ego_veh, 'snap_entry_perc_speed', np.nan))
                        result_d['entry_true_speed'].append(getattr(ego_veh, 'snap_entry_true_speed', np.nan))
