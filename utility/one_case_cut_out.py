...
def run_one_case(use_model, **kwargs):
    ...
    noise = mvt.make_perception_noise()
    if mvt.noise_active():
        noise.record()
    ...
    for i in range(iterations - 1):
        ...
        if cut_out_veh.crash:
            mvt.control(ego_veh, cut_out_veh, freq, check, react, i, perception=noise)

        elif cut_out_veh.pos_profile_lat[i] < wandering_zone:
            mvt.control(ego_veh, stopped_veh, freq, check, react, i, perception=noise)
    ...
    if mvt.write_perception_trace(
            noise, 'results/perception_trace_cut_out_' + str(use_model) + '.csv', freq):
        print('Perception trace written to results/perception_trace_cut_out_'
              + str(use_model) + '.csv')
