...
def run_loop():
    ...
    out_dir = 'results/cut_out' + mvt.noise_suffix()
            ...
            for dx0_f in front_distances:
                for lateral_speed_cut_out in lateral_velocities:
                    ...
                    noise = mvt.make_perception_noise(seed=(
                        gp.perception_noise_seed, int(ur), int(dx0_f),
                        int(round(abs(lateral_speed_cut_out) * 1000))))
                    ...
                    # Perceived vs true state of the perceived actor at the
